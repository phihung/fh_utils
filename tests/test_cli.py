...
def test_dev(mock: MagicMock):
    ...
    assert repr(mock.call_args_list[0]) == snapshot(
        "call(path=Path('src/app.py'), app='app', host='0.0.0.0', port=8000, reload=<ReloadType.FAST: 'fast'>, log_level='error', reload_includes=('src',))"
    )
...
def test_dev__default(mock: MagicMock):
    ...
    runner.invoke(app, shlex.split(cmd))
    assert repr(mock.call_args_list[0]) == snapshot(
        "call(path=Path('src/app.py'), app='app', host='0.0.0.0', port=5001, reload=<ReloadType.FAST: 'fast'>)"
    )
    mock.assert_called_once()
